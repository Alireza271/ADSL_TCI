...
class ServicePageDialog extends StatefulWidget {
  String serial;

  ServicePageDialog(this.serial);

  @override
  _ServicePageDialog createState() =>
      _ServicePageDialog(this.serial);
}

class _ServicePageDialog extends State<ServicePageDialog> {
  ServiceDitailsModelEntity _service_ditails =
  new ServiceDitailsModelEntity();
  ...
  StartState() async {
    _service_ditails = await GetServiceDitails(serial);
    setState(() {});
  }
      ...
      case true:
        {
          return Column(
            children: <Widget>[
              ...
              row_text("نام سرویس", _service_ditails.result.service.name),
              SizedBox(
                height: 15,
              ),
              row_text("توضیحات",
                ...
                onPressed: () async {
                  if (selected_bank == null) {
                    ...
                    if (pay_state) {
                      ...
                      BankRedirector = await GetBankRedirector("service",
